...
describe('Supervised Router (real providers)', () => {
  ...
  test('routes Anthropic -> OpenAI across stages with real calls', async () => {
    ...
    const getNewModelSpy = jest.spyOn(StandardGraph.prototype as any, 'getNewModel');
    ...
    history.push(new HumanMessage('Summarize the previous response in one sentence.'));
    await run.processStream({ messages: history }, configV2);

    const providersInvoked = getNewModelSpy.mock.calls.map((call: any[]) => call[0]?.provider as string);
  ...
  test('routes Anthropic -> OpenAI -> OpenRouter across three stages', async () => {
    ...
    const run = await Run.create<t.IState>({
      ...
      graphConfig: {
        ...
        models: {
          reasoning: { provider: Providers.ANTHROPIC, model: 'claude-3-5-sonnet-20240620', streaming: true, streamUsage: true },
          summarize: { provider: Providers.OPENAI, model: 'gpt-4.1', streaming: true, streamUsage: true },
    ...
    const configV2 = { configurable: { thread_id: 'router-real-thread-2' }, streamMode: 'values', version: 'v2' as const };
    const history: HumanMessage[] = [];
    ...
    const providersInvoked = getNewModelSpy.mock.calls.map((call: any[]) => call[0]?.provider as string);
    expect(providersInvoked).toEqual(
      expect.arrayContaining([Providers.ANTHROPIC, Providers.OPENAI, Providers.OPENROUTER])
    );
