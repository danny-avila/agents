...
describe('Supervised router loop with toolEnd=false (real provider)', () => {
  ...
  test('TOOLS -> ROUTER -> AGENT loop occurs (two agent turns)', async () => {
    ...
    const customHandlers = {
      [Providers as unknown as string]: undefined,
    } as unknown as Record<string, t.EventHandler>;
    ...
    const inputs = { messages: [new HumanMessage('Use the calculator tool to compute 2+2, then summarize in one short sentence.')] };
    const config_ = {
