...
async function runFanOutDemo(): Promise<void> {
  ...
  const configV2 = {
    configurable: { thread_id: 'fanout-demo-1' },
  ...
  conversationHistory.push(
    new HumanMessage(
      'Summarize the tradeoffs of using fan-out vs a single model.'
    )
  );
  const parts = await run.processStream(
