...
export class Run<_T extends t.BaseGraphState> {
  ...
  async processStream(
    ...
    if (langfuseEnabled) {
      ...
      const agentId: string | undefined = config.configurable?.agent_id;
      const traceMetadata = {
        messageId: this.id,
  ...
  async generateTitle({
    ...
    if (
      ...
      const traceMetadata = {
        ...
        agentId: chainOptions.configurable?.agent_id,
        agentName: chainOptions.configurable?.agent_name,
      };
