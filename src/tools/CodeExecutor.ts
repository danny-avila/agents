...
/**
 * Render the post-execution file summary: split the response files into
 * generated (newly produced this turn) and inherited (read-only inputs
 * passed through from prior turns / skill priming), render each as its
 * own section, and append the LLM-facing notes about each category.
 *
 * Returns an empty string when `files` is absent or empty so callers
 * can `formatted += renderExecutionFileSummary(result.files)` without
 * a guard. Single source of truth for the four codeapi-talking tools
 * (CodeExecutor, BashExecutor, ProgrammaticToolCalling,
 * BashProgrammaticToolCalling) — keep changes here, not at the
 * callsites.
 */
export function renderExecutionFileSummary(
  files: t.FileRefs | undefined
): string {
  if (!files || files.length === 0) return '';

  const inheritedFiles = files.filter((f) => f.inherited === true);
  const generatedFiles = files.filter((f) => f.inherited !== true);

  let out = '';
  out += renderFileSection(generatedFilesHeader, generatedFiles, otherMessage);
  out += renderFileSection(
