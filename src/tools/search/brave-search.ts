...
export const createBraveAPI = (
  ...
  ): Promise<t.BraveWebSearchResponse> => {
  ...
  const getSources = async ({
    ...
    try {
      ...
      const organicResults = convertResponseToOrganicResult(data.web?.results);
      const newsResults = convertResponseToNewsResult(data.news?.results);
      const videoResults = convertResponseToVideoResult(data.videos?.results);
